...
def find_overlapping_files(path='', all_files=False, target_dir=''):
    if all_files:
        files = glob.glob(path + '*.xml.gz')
    else:
        files = glob.glob(path + '.xml.gz') # this will be lang1-lang2.xml.gz
    ...
    out_file = os.path.join(target_dir, '{}_file_ids'.format(path)) if not all_files else 'all_file_ids'
...
def copy_files(overlaps, source_dir, target_dir, lang):
    ...
    with zipfile.ZipFile("en.zip") as z_en, zipfile.ZipFile("{}.zip".format(lang)) as z_other: # currently only works for bitext, easy to extend
...
if __name__ == "__main__":
    ...
    all_languages = ["zh", "eu", "ar", "fi", "id", "ta", "ru", "de", "el", "es"] # everything will be alphabetical
    ...
    for lang in all_languages:
        ...
        os.makedirs(os.path.join(args.target_dir, lang_pair), exist_ok=True)
        overlaps = find_overlapping_files(lang_pair, target_dir=args.target_dir)
        copy_files(overlaps, args.source_dir, args.target_dir, lang)
